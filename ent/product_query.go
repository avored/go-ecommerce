...
func (pq *ProductQuery) QueryCartItems() *CartItemQuery {
	query := &CartItemQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) sqlAll(ctx context.Context) ([]*Product, error) {
	...
	if query := pq.withCartItems; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.CartItems = []*CartItem{}
		}
		query.Where(predicate.CartItem(func(s *sql.Selector) {
			s.Where(sql.InValues(product.CartItemsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.CartItems = append(node.Edges.CartItems, n)
		}
	}
	...
	if query := pq.withOrderProducts; query != nil {
		...
		query.Where(predicate.OrderProduct(func(s *sql.Selector) {
			s.Where(sql.InValues(product.OrderProductsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
