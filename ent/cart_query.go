...
func (cq *CartQuery) QueryCartItems() *CartItemQuery {
	query := &CartItemQuery{config: cq.config}
...
func (cq *CartQuery) sqlAll(ctx context.Context) ([]*Cart, error) {
	...
	if query := cq.withCartItems; query != nil {
		...
		query.Where(predicate.CartItem(func(s *sql.Selector) {
			s.Where(sql.InValues(cart.CartItemsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "cart_id" returned %v for node %v`, fk, n.ID)
			}
