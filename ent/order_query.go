...
type OrderQuery struct {
	...
	withCustomerOrder   *CustomerQuery
...
// QueryCustomerOrder chains the current query on the "customer_order" edge.
func (oq *OrderQuery) QueryCustomerOrder() *CustomerQuery {
...
func (oq *OrderQuery) QueryOrderProducts() *OrderProductQuery {
	query := &OrderProductQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withCustomerOrder:   oq.withCustomerOrder.Clone(),
...
// WithCustomerOrder tells the query-builder to eager-load the nodes that are connected to
// the "customer_order" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithCustomerOrder(opts ...func(*CustomerQuery)) *OrderQuery {
	query := &CustomerQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
	oq.withCustomerOrder = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context) ([]*Order, error) {
	...
	if query := oq.withShippingAddress; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "shipping_address_id" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := oq.withBillingAddress; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "billing_address_id" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := oq.withCustomerOrder; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Order)
		for i := range nodes {
	...
	if query := oq.withOrderProducts; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.OrderProducts = []*OrderProduct{}
		}
		query.Where(predicate.OrderProduct(func(s *sql.Selector) {
			s.Where(sql.InValues(order.OrderProductsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.OrderProducts = append(node.Edges.OrderProducts, n)
		}
	}
