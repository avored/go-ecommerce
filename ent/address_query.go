...
func (aq *AddressQuery) sqlAll(ctx context.Context) ([]*Address, error) {
	...
	if query := aq.withShippingOrders; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.ShippingOrders = []*Order{}
		}
		query.Where(predicate.Order(func(s *sql.Selector) {
			s.Where(sql.InValues(address.ShippingOrdersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "shipping_address_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.ShippingOrders = append(node.Edges.ShippingOrders, n)
		}
	}
	...
	if query := aq.withBillingOrders; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.BillingOrders = []*Order{}
		}
		query.Where(predicate.Order(func(s *sql.Selector) {
			s.Where(sql.InValues(address.BillingOrdersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.BillingAddressID
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "billing_address_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.BillingOrders = append(node.Edges.BillingOrders, n)
		}
	}
